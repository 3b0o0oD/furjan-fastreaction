                ...
                if hasattr(self, screen_attr):
                    ...
                    if screen:
                        try:
                            ...
                            # Clean up background movie (CRITICAL for preventing lag)
                            if hasattr(screen, 'background_movie') and screen.background_movie:
                                screen.background_movie.stop()
                                try:
                                    screen.background_movie.frameChanged.disconnect()
                                    screen.background_movie.finished.disconnect()
                                except (AttributeError, TypeError, RuntimeError):
                                    pass
                                screen.background_movie.deleteLater()
                                screen.background_movie = None
                            
                            # Also clean up legacy movie if present (Active_screen)
                            if hasattr(screen, 'movie') and screen.movie:
                                try:
                                    screen.movie.stop()
                                    try:
                                        screen.movie.frameChanged.disconnect()
                                        screen.movie.finished.disconnect()
                                    except (AttributeError, TypeError, RuntimeError):
                                        pass
                                    screen.movie.deleteLater()
                                except Exception:
                                    pass
                                screen.movie = None
                            
                            # Clear background label
