...
class Final_Screen(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Home):
        ...
        self.teamscore_qml.setAttribute(QtCore.Qt.WA_AlwaysStackOnTop, False)
...
class Active_screen(QWidget):
    ...
    def init_qml_widget(self, parent_widget):
        ...
        try:
            ...
            self.qml_widget.setAttribute(Qt.WA_AlwaysStackOnTop, False)
    ...
    def start_reconnection_attempts(self):
        ...
        if hasattr(self, 'connection_warning_label') and self.connection_warning_label:
            try:
                ...
                if not hasattr(self, 'warning_blink_timer') or not self.warning_blink_timer:
        ...
        if hasattr(self, 'reconnection_timer') and self.reconnection_timer:
    ...
    def attempt_reconnection(self):
        ...
        try:
            ...
            if success:
                ...
                # Stop blinking timer
                if hasattr(self, 'warning_blink_timer') and self.warning_blink_timer:
    ...
    def setupUi(self, MainWindow):
        ...
        # Ensure background stays behind overlays
    ...
    def start_game(self):
        ...
        # Check serial connection and start game only if connected
        if self.serial_thread:
            ...
            logger.warning("⚠️ Serial thread not available, starting game without serial")
            # Start game anyway if no serial thread is expected
            try:
                if hasattr(self, 'fastreaction_backend') and self.fastreaction_backend and self.qml_widget_initialized:
                    self.fastreaction_backend.set_timer_seconds(self.countdown_time)
                    self.fastreaction_backend.sync_timer_state(self.countdown_time, True)
...
class TeamMember_screen(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Home):
        ...
        try:
            ...
            self.teamname_qml.setAttribute(QtCore.Qt.WA_AlwaysStackOnTop, False)
...
class Home_screen(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Home):
        ...
        try:
            ...
            try:
                self.leaderboard_qml.statusChanged.connect(self._log_leaderboard_qml_status)
            except Exception:
                pass

            global lastplayed, lastplayed_score, lastplayed_weighted_points, lastplayed_rank
            ...
            try:
                ...
                if root_obj:
                    ...
                    # Seed initial data from API list
                    sorted_data = sorted(list_top5_FastReaction, key=lambda it: it[2], reverse=True)[:5]
            ...
            self.leaderboard_qml.lower()
        except Exception as e:
            logger.error(f"Failed to initialize leaderboard QML: {e}")
    ...
    def _log_leaderboard_qml_status(self, status):
        try:
            if status == QQuickWidget.Error:
                errors = self.leaderboard_qml.errors()
                logger.error(f"Leaderboard QML errors: {errors}")
            elif status == QQuickWidget.Ready:
                logger.info("Leaderboard QML loaded successfully")
        except Exception:
            pass
    ...
    def hideTable(self):
        try:
            if hasattr(self, 'leaderboard_qml') and self.leaderboard_qml:
                self.leaderboard_qml.hide()
                logger.info("Leaderboard table hidden")
        except Exception as e:
            logger.error(f"Error hiding leaderboard: {e}")
