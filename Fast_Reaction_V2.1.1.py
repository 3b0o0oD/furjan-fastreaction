...
class Active_screen(QWidget):
    ...
    def __init__(self, serial_thread=None):
        ...
        # Initialize reconnection timer for serial connection
        self.reconnection_timer = None
        self.reconnection_attempts = 0
        self.warning_blink_timer = None
    ...
    def start_reconnection_attempts(self):
        """Start attempting to reconnect to the serial device (non-blocking)"""
        # Show warning label if it exists
        if hasattr(self, 'connection_warning_label') and self.connection_warning_label:
            try:
                self.connection_warning_label.raise_()  # Bring to top
                self.connection_warning_label.show()
                logger.info("⚠️ Warning label shown")
                
                # Start blinking animation
                if not self.warning_blink_timer:
                    self.warning_blink_timer = QtCore.QTimer()
                    self.warning_blink_timer.timeout.connect(self._toggle_warning_visibility)
                    self.warning_blink_timer.start(500)  # Blink every 500ms
                    
            except Exception as e:
                logger.error(f"Error showing warning label: {e}")
        else:
            logger.warning("⚠️ Warning label will be shown once UI is initialized")
        ...
        # Create and start reconnection timer (try every 2 seconds)
        self.reconnection_timer = QtCore.QTimer()
        self.reconnection_timer.timeout.connect(self.attempt_reconnection)
        self.reconnection_timer.start(2000)
        
        logger.info("🔄 Starting reconnection attempts (every 2 seconds)...")
    
    def _toggle_warning_visibility(self):
        """Toggle warning label visibility for blinking effect"""
        if hasattr(self, 'connection_warning_label') and self.connection_warning_label:
            try:
                self.connection_warning_label.setVisible(not self.connection_warning_label.isVisible())
            except Exception:
                pass
    
    def attempt_reconnection(self):
        """Attempt to reconnect to the serial device (non-blocking)"""
        if not self.serial_thread:
            logger.warning("⚠️ Serial thread not initialized")
            return
        
        self.reconnection_attempts += 1
        logger.info(f"🔄 Reconnection attempt #{self.reconnection_attempts}...")
        
        # Show warning label if it now exists (raise to top to ensure visibility)
        if hasattr(self, 'connection_warning_label') and self.connection_warning_label:
            try:
                self.connection_warning_label.raise_()
                self.connection_warning_label.show()
            except Exception as e:
                logger.debug(f"Warning label show error: {e}")
        
        try:
            # Try to connect (non-blocking)
            success = self.serial_thread.connect()
            ...
            if success:
                ...
                # Start the game if it was waiting for connection
                global gameStarted
                if gameStarted:
                    logger.info("🎮 Starting game timers after successful reconnection...")
                ...
                logger.warning(f"️ Reconnection attempt #{self.reconnection_attempts} failed")
                
        except Exception as e:
            logger.error(f" Error during reconnection attempt #{self.reconnection_attempts}: {e}")
    
    def stop_reconnection_attempts(self):
        """Stop reconnection attempts and cleanup"""
        # Stop reconnection timer
        if self.reconnection_timer:
            self.reconnection_timer.stop()
            self.reconnection_timer = None
    ...
    def setupUi(self, MainWindow):
        ...
        # Create connection warning label (initially hidden)
        self.connection_warning_label = QtWidgets.QLabel(self.centralwidget)
        self.connection_warning_label.setGeometry(QtCore.QRect(
            int(50 * self.scale),
            int(50 * self.scale),
            int(300 * self.scale),
            int(80 * self.scale)
        ))
        self.connection_warning_label.setText("⚠️ SERIAL PORT \nNOT CONNECTED")
        self.connection_warning_label.setAlignment(QtCore.Qt.AlignCenter)
        self.connection_warning_label.setStyleSheet("""
            QLabel {
                background-color: rgba(255, 255, 0, 200);
                color: black;
                font-weight: bold;
                font-size: """ + str(int(18 * self.scale)) + """px;
        ...
        # Raise warning label above QML widget to ensure it's always visible
        if hasattr(self, 'connection_warning_label') and self.connection_warning_label:
            self.connection_warning_label.raise_()
            logger.debug("Warning label raised to top layer")
    ...
    def start_game(self):
        ...
        # Start serial monitoring if available (non-blocking)
        if self.serial_thread:
            logger.info(" Serial thread connecting...")
            # Try once immediately
            if self.serial_thread.connect():
                logger.info(" Serial thread connected successfully")
                self.serial_thread.start_monitoring()
                ...
                # If initial connection fails, start non-blocking reconnection attempts
                logger.warning("️  Serial thread connection failed, starting non-blocking reconnection...")
    ...
    def _cleanup_timers(self):
        """Clean up all timers with proper signal disconnection"""
        timer_attrs = ['timer', 'TimerGame', 'deactivate_timer', 'reconnection_timer', 'warning_blink_timer']
        for timer_attr in timer_attrs:
            if hasattr(self, timer_attr):
                timer = getattr(self, timer_attr)
                if timer is not None:
    ...
    def _cleanup_qml_components(self):
        ...
        # Clean up QML backend - DON'T disconnect signals, QML handles them automatically
        if hasattr(self, 'fastreaction_backend') and self.fastreaction_backend:
            try:
                # Stop countdown first
                self.fastreaction_backend.stop_countdown()
                
                # DON'T disconnect signals - QML's property system manages them automatically
                # Disconnecting breaks signal connections when recreating the widget
                # The backend will be reconnected when init_qml_widget() is called again
                
                # DON'T set to None - preserve backend for reuse (like CubeRoomBall)
                # self.fastreaction_backend = None  # REMOVED - keep backend alive
                
                logger.debug("✅ FastReaction backend stopped (preserved for reuse, signals intact)")
            except Exception as e:
                logger.warning(f"⚠️ Error cleaning up FastReaction backend: {e}")
    ...
    def _cleanup_ui_components(self):
        ...
        # Clean up connection warning label
        if hasattr(self, 'connection_warning_label') and self.connection_warning_label:
            try:
                self.connection_warning_label.hide()
                self.connection_warning_label.deleteLater()
                self.connection_warning_label = None
                logger.debug("Connection warning label cleaned up")
            except (RuntimeError, AttributeError):
                logger.debug("Warning label already deleted by Qt")
            except Exception as e:
                logger.warning(f"Error cleaning connection_warning_label: {e}")
                ...
                if hasattr(self, screen_attr):
                    ...
                    if screen:
                        try:
                            ...
                            # Clean up backends - PRESERVE for reuse, just stop them
                            backends_to_preserve = ['fastreaction_backend']
                            backends_to_delete = ['team_backend', 'teamscore_backend', 'leaderboard_backend']
                            
                            # Stop backends that will be preserved (don't delete them)
                            for backend_attr in backends_to_preserve:
                                if hasattr(screen, backend_attr):
                                    ...
                                    if backend:
                                        try:
                                            ...
                                            # DON'T set to None - preserve for reuse
                                            logger.debug(f"✅ {screen_name} {backend_attr} stopped (preserved for reuse)")
                                        except Exception as e:
                                            logger.warning(f"⚠️ Error stopping {screen_name} {backend_attr}: {e}")
                            
                            # Delete backends that won't be reused
                            for backend_attr in backends_to_delete:
                                if hasattr(screen, backend_attr):
                                    backend = getattr(screen, backend_attr)
                                    if backend:
                                        try:
                                            ...
                                            logger.debug(f"🗑️ {screen_name} {backend_attr} deleted")
                                        except Exception as e:
                                            logger.warning(f"⚠️ Error cleaning up {screen_name} {backend_attr}: {e}")
...
# Safely close home screen
                ...
                # Force update backend values after setupUi to ensure QML displays them
                # (setupUi already calls init_qml_widget internally, no need to call again)
                if hasattr(self.ui_active, 'fastreaction_backend') and self.ui_active.fastreaction_backend:
                    try:
                        global teamName, scored, correct_count, miss_count, wrong_count, TimerValue
                        logger.info(f"Force updating QML backend - Team: {teamName}, Score: {scored}, Correct: {correct_count}, Wrong: {wrong_count}, Miss: {miss_count}")
                        
                        # Force set all values to ensure QML updates
                        self.ui_active.fastreaction_backend.set_team_name(teamName)
                        self.ui_active.fastreaction_backend.set_score_value(str(scored))
                        self.ui_active.fastreaction_backend.set_correct_count(str(correct_count))
                        self.ui_active.fastreaction_backend.set_wrong_count(str(wrong_count))
                        self.ui_active.fastreaction_backend.set_miss_count(str(miss_count))
                        self.ui_active.fastreaction_backend.set_timer_seconds(TimerValue // 1000)
                        
                        # Force timer sync to update QML display
                        self.ui_active.fastreaction_backend.force_timer_sync()
                        logger.info(" QML backend values force-updated successfully")
                        
                        # Ensure QML widget is visible
                        if hasattr(self.ui_active, 'qml_widget') and self.ui_active.qml_widget:
                            self.ui_active.qml_widget.show()
                            self.ui_active.qml_widget.update()
                            logger.debug(" QML widget shown and updated")
                    except Exception as e:
                        logger.error(f" Error force-updating QML backend: {e}")
